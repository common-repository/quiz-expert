...
<div class="quexp-q"><?php echo wp_kses_post( $q_sl.".&nbsp;&nbsp;&nbsp;");?><a id="qname_mcq_val<?php echo esc_attr( $q_id);?>" style="color:#393939"><?php echo wp_kses_post($question )?></a>&nbsp;&nbsp;
</div>
...
<a href="#TB_inline?width=600&height=550&inlineId=edit_question_popup" title="Edit Question" class="thickbox" onclick="quexp_edit_mcq_question_button(<?php echo esc_attr( $q_id); ?>);"> 
...
<div id="mcq_answer_option<?php echo esc_attr( $q_id);?>">
<?php
   $option_sl=1;
   foreach($mcq_option as $option){ ?>
    <div class='quexp-opt'><strong><?php Inc\Base\QuexpOptions::quexp_option( $option_sl);?></strong>.&nbsp;&nbsp;&nbsp;<span name="print_options<?php echo esc_attr( $q_id);?>[]"><?php echo wp_kses_post( $option);?></span></div>
<?php  $option_sl++;
    }
?>
</div>
<br/>
<div>Answer: <a class="row-title" id="answer_mcq_val<?php echo esc_attr( $q_id);?>"><?php echo wp_kses_post( $answer);?></a></div>
<?php if ($explaination !=""){ ?>
<div id="explain_div<?php echo esc_attr( $q_id);?>">Explaination: <a class="row-title" id="explaination_mcq_val<?php echo esc_attr( $q_id);?>"><?php echo wp_kses_post( $explaination);?></a></div>
<?php } else {  ?>
